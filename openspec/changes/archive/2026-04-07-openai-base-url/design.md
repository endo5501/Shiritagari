...
現在 `ollama_base_url` は専用パラメータとして渡されている。`openai_base_url` も追加するとパラメータが増えすぎるため、ファクトリ関数に `&LlmConfig` を渡す形にリファクタリングした。

- `create_provider(provider_name, model, api_key_env, ollama_base_url)` → `create_provider(provider_name, model, api_key_env, &LlmConfig)`
- プロバイダ固有フィールド（`ollama_base_url`, `openai_base_url`）は各 `match` アーム内で `config` から直接取得
- 今後プロバイダが増えてもシグネチャが変わらない

**代替案**: 個別パラメータとして `openai_base_url` を追加する → パラメータスプロールが悪化し、プロバイダ固有フィールドが汎用インターフェースに漏洩するため不採用。
